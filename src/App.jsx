...
function PageDashboard(p){
  ...
  return<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"repeat(auto-fit,minmax(280px,1fr))",gap:14,marginBottom:14}}>
      ...
      <Card><div style={{fontWeight:700,color:"#1e293b",marginBottom:12,fontSize:13}}>7-Day Trend — Tickets by Type</div><ResponsiveContainer width="100%" height={180}><BarChart data={(function(){return Array.from({length:7},function(_,i){var d=new Date(Date.now()-(6-i)*86400000);var obj={lbl:d.toLocaleDateString("en",{weekday:"short"})};ticketTypes.forEach(function(tt){obj[tt.name]=tickets.filter(function(t){return t.typeId===tt.id&&new Date(t.createdAt).toDateString()===d.toDateString();}).length;});return obj;})();})()} margin={{top:4,right:4,left:-16,bottom:0}}><CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9"/><XAxis dataKey="lbl" tick={{fontSize:9}}/><YAxis tick={{fontSize:9}} allowDecimals={false}/><Tooltip/><Legend wrapperStyle={{fontSize:9}}/>{ticketTypes.map(function(tt,i){return<Bar key={tt.id} dataKey={tt.name} stackId="a" fill={tt.color||PAL[i%PAL.length]} radius={i===ticketTypes.length-1?[4,4,0,0]:[0,0,0,0]}/>;})}</BarChart></ResponsiveContainer></Card>
    </div>
